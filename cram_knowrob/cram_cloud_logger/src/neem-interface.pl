...
mem_episode_start(Action, EnvOwl,
EnvOwlIndiName,
EnvUrdf,
EnvUrdfPrefix,
AgentOwl,
...
%add_subaction_with_task(Action,SubAction,TaskType) :- execution_agent(Agent), kb_project([has_type(Role, soma:'AgentRole'), has_role(Agent,Role), is_action(SubAction), has_type(Task,TaskType), executes_task(SubAction,Task), has_subevent(Action,SubAction),is_performed_by(SubAction,Agent)]),notify_synchronize(event(Event)), !.

add_subaction_with_task(Action,SubAction,TaskType) :- execution_agent(Agent),kb_project([is_action(SubAction), has_type(Task,TaskType), executes_task(SubAction,Task), has_subevent(Action,SubAction), is_performed_by(SubAction,Agent)]),%notify_synchronize(event(Event)),
!.

mem_event_end(Event) :- execution_agent(Agent),get_time(CurrentTime), ask([triple(Event,dul:'hasTimeInterval',TimeInterval), triple(TimeInterval,soma:'hasIntervalBegin', Start), executes_task(Event,Task)]),kb_unproject(TimeInterval, soma:'hasIntervalEnd', _),kb_project([holds(TimeInterval, soma:'hasIntervalEnd', CurrentTime),has_type(Role, soma:'AgentRole'), has_role(Agent,Role) during Event,task_role(Task, Role)]),!.
...
%add_participant_with_role(Action, ObjectId, RoleType) :- ask(executes_task(Action, Task)), kb_project([has_participant(Action,ObjectId), has_type(Role, RoleType), has_role(ObjectId,Role) during [0.0,0.0]]).
